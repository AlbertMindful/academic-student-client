  ...
  try {
    const result = await withPersistentAcademicLogin(req, async (adapter) => {
      ...
      const [schedule, exams, officialGpa] = await Promise.all([
        current ? adapter.getSchedule(current.id).catch(() => []) : Promise.resolve([]),
        current ? adapter.getExams(current.id).catch(() => []) : Promise.resolve([]),
        adapter.getOfficialGpa ? adapter.getOfficialGpa().catch(() => null) : Promise.resolve(null),
      ]);
      const teachingWeek = current && current.startDate
        ? { current: computeTeachingWeek(current.startDate, now), startDate: current.startDate, endDate: current.endDate }
        : null;
      const nextClass = teachingWeek ? findNextClass(schedule, teachingWeek.startDate, now) : null;
      const today = teachingWeek ? todayClasses(schedule, teachingWeek.startDate, now) : [];
      ...
      const data: DashboardData = {
        ...
        todayClasses: today,
        upcomingExams,
        gpa: computeGpa(grades, officialGpa),
      };
      return data;
    });
