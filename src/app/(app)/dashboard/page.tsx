  ...
  return (
    <div className="mx-auto max-w-5xl pb-16">
      ...
      <Dialog open={Boolean(selected)} onOpenChange={(open) => !open && setSelected(null)}>
        <DialogContent className="sm:max-w-lg">{selected && <><DialogHeader><DialogTitle>{selected.title}</DialogTitle><DialogDescription>{eventMeta(selected) || selected.status || "暂无明确时间"}</DialogDescription></DialogHeader>{selected.summary && <p className="whitespace-pre-line text-sm leading-6">{selected.summary}</p>}{selected.conflicts?.length ? <div className="rounded-lg border border-amber-500/20 bg-amber-500/[0.06] p-3 text-xs leading-5 text-amber-800 dark:text-amber-200"><div className="font-medium">来源信息存在差异，已采用教务系统数据</div>{selected.conflicts.map((conflict) => <div key={conflict.field} className="mt-1 opacity-80">{conflict.field === "date" ? "日期" : conflict.field === "time" ? "时间" : conflict.field === "location" ? "地点" : "状态"}：教务系统 {conflict.academicValue ?? "未提供"}；其他来源 {conflict.otherValue ?? "未提供"}</div>)}</div> : null}<div className="space-y-2 border-t pt-4"><div className="text-xs font-medium text-muted-foreground">来源</div><div className="flex flex-wrap gap-2">{selected.sources.map((source) => source.url ? <a key={`${source.provider}:${source.sourceId}`} href={source.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded-md bg-secondary px-2.5 py-1 text-xs text-secondary-foreground transition-colors hover:bg-secondary/75">{source.providerLabel}<ExternalLink className="h-3 w-3" /></a> : <Badge key={`${source.provider}:${source.sourceId}`} variant="secondary">{source.providerLabel}</Badge>)}</div>{selected.merge && <p className="text-xs text-muted-foreground">{selected.merge.reason}（{Math.round(selected.merge.confidence * 100)}%）</p>}</div></>}</DialogContent>
      </Dialog>
    </div>
  );
}
