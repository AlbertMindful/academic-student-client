  ...
  return (
    <div className="mx-auto max-w-3xl pb-16">
      ...
      <section className="space-y-1">
        ...
        {(payload?.providers ?? []).map((provider) => <div key={provider.provider} className="flex items-center gap-3 border-b border-border/60 py-3.5 last:border-0"><span className={cn("h-2 w-2 rounded-full", provider.status === "ok" ? "bg-emerald-500" : provider.status === "not_connected" ? "bg-muted-foreground/40" : "bg-amber-500")} /><div className="min-w-0 flex-1"><div className="text-sm font-medium">{provider.label}</div><div className="mt-0.5 text-xs text-muted-foreground">{provider.message}{provider.lastSuccessAt ? ` · 最近成功 ${formatter.format(new Date(provider.lastSuccessAt))}` : ""}</div></div>{provider.provider === "chaoxing" ? <ChaoxingConnect onConnected={() => void sync()} /> : provider.status === "ok" ? <Badge variant="outline">正常</Badge> : <Button asChild variant="outline" size="sm"><Link href="/login">重新连接</Link></Button>}</div>)}
      </section>
